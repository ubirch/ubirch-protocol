 ...
 * @param buf the buffer with the data to sign
 * @param len the length of the data buffer
 * @param signature the buffer to hold the returned signature
 * @return 0 on success
 * @return -1 if the signing failed
 */
static int ed25519_sign(const unsigned char *buf, size_t len, unsigned char signature[crypto_sign_BYTES]);
...
inline int ed25519_sign(const unsigned char *buf, const size_t len,
    ...
    // sign the message
    crypto_sign(sm, &mlen, buf, (crypto_uint16) len, ed25519_secret_key);
...
inline int ed25519_verify(const unsigned char *buf, const size_t len,
    ...
    memcpy(sm + crypto_sign_BYTES, buf, len);

    // verify signature
    int ret = crypto_sign_open(m, &mlen, sm, smlen, ed25519_public_key);
